...
Non-Preemptive means, once a process is assigned a CPU, it will free the CPU only after termination(or at any input/output request)
...
void completionTime(int num, int mat[][7]){
    ...
    while(completed != num){
        ...
        //assigning CPU to the maxPrior process
        currTime += mat[maxPriorAmongArrived][2];
        mat[maxPriorAmongArrived][4] = currTime;
        completed++;
        ready[maxPriorAmongArrived] = -1;

        //in case there are no processes, 
        //the currTime should become equal to the arrival time of next process
